...
// bean作用域
type BeanScope int32

const (
	// 单例模式，默认值，在容器中只实例化一次，引用传递
	SingletonBeanScope BeanScope = iota
	// 原型模式，注入多少次bean，浅拷贝对象多少次，类似值传递
	PrototypeBeanScope
)
...
// SetScope 设置作用域
func (d *BeanDefinition) SetScope(scope BeanScope) {
	d.scope = scope
}

// GetScope 获取作用域
func (d *BeanDefinition) GetScope() BeanScope {
	return d.scope
}
