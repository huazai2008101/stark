...
// Package stark 实现了 stark 的核心骨架，包含 IoC 容器、基于 IoC 容器的 App
...
import (
	...
	"github.com/huazai2008101/stark/ioc/cond"
...
// container 是 stark 框架的基石，实现了 Martin Fowler 在 << Inversion
...
// stark 严格区分了这两种概念，在描述对 bean 的处理时要么单独使用依赖注入或属
// 性绑定，要么同时使用依赖注入和属性绑定。
...
func (s *wiringStack) pushBack(b *BeanDefinition) {
	log.Tracef(context.Background(), "push %s %s", b, getStatusString(b.status))
...
func (s *wiringStack) popBack() {
	...
	b := s.beans[n-1]
	s.beans = s.beans[:n-1]
	log.Tracef(context.Background(), "pop %s %s", b, getStatusString(b.status))
}
		...
		return func() {
			if f == nil {
				...
				if len(out) > 0 && !out[0].IsNil() {
					log.Error(context.Background(), out[0].Interface().(error))
				}
			}
		}
	}
...
func (c *container) Refresh(opts ...internal.RefreshOption) (err error) {
	...
	log.Infof(c.ctx, "refresh %d beans cost %v", len(beansById), cost)

	if optArg.AutoClear {
		c.clear()
	}

	log.Info(c.ctx, "container refreshed successfully")
	return nil
}

func (c *container) registerBean(b *BeanDefinition) {
	log.Debugf(c.ctx, "register %s name:%q type:%q %s", b.getClass(), b.BeanName(), b.Type(), b.FileLine())
	...
	for _, t := range b.exports {
		log.Debugf(c.ctx, "register %s name:%q type:%q %s", b.getClass(), b.BeanName(), t, b.FileLine())
...
func (c *container) getBean(v reflect.Value, tag wireTag, stack *wiringStack) error {
	...
	if t.Kind() == reflect.Interface && tag.beanName != "" {
		for _, b := range c.beansByName[tag.beanName] {
			...
			if !found {
				foundBeans = append(foundBeans, b)
				log.Warnf(c.ctx, "you should call Export() on %s", b)
			}
		}
	}
	...
	// 如果是单例模式直接赋值
	if result.scope == SingletonBeanScope {
		v.Set(result.Value())
		return nil
	}

	// 如果是原型模式则重新拷贝一个对象
	c.copyValue(v, result)
	return nil
}

// 浅拷贝bean对象
func (c *container) copyValue(v reflect.Value, bean *BeanDefinition) {
	srcType := bean.Type().Elem()
	dstValue := reflect.New(srcType)
	dstValue.Elem().Set(bean.Value().Elem())
	v.Set(dstValue)
}
...
func (c *container) Close() {
	...
	log.Info(c.ctx, "goroutines exited")

	for _, f := range c.destroyers {
		f()
	}

	log.Info(c.ctx, "container closed")
}
...
func (c *container) Go(fn func(ctx context.Context)) {
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				log.Panic(c.ctx, fmt.Sprintf("panic:%v ", r), util.PanicStack())
			}
		}()
		fn(c.ctx)
	}()
