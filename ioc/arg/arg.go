...
func (r *argList) getArg(ctx Context, arg Arg, t reflect.Type, fileLine string) (reflect.Value, error) {
	...
	log.Tracef(myCtx, "get value %s", description)
	defer func() {
		if err == nil {
			log.Tracef(myCtx, "get value success %s", description)
		} else {
			log.Tracef(myCtx, "get value error %s %s", err.Error(), description)
		}
	}()
...
func (arg *optionArg) call(ctx Context) (reflect.Value, error) {
	...
	log.Tracef(myCtx, "call option func %s", arg.r.fileLine)
	defer func() {
		if err == nil {
			log.Tracef(myCtx, "call option func success %s", arg.r.fileLine)
		} else {
			log.Tracef(myCtx, "call option func error %s %s", err.Error(), arg.r.fileLine)
		}
	}()
